...
#include "data/rtcdatachannelevent.h"
...
void PeerConnection::Run(uv_async_t* handle, int status)
{
  ...
  while(true)
  {
    ...
    if (PeerConnection::CREATE_SESSION_DESCRIPTION_OBSERVER & evt.type) {
      ...
      std::cout << "FOOZLEBOOZEL 1" << std::endl;
      ...
      webrtc::MediaStreamInterface* media_stream_interface = static_cast<webrtc::MediaStreamInterface*>(evt.data);
      media_stream->setStream(media_stream_interface);
      Local<Function> callback = Local<Function>::Cast(pc->Get(Nan::New("onaddstream").ToLocalChecked()));
      if (!callback.IsEmpty()) {
        std::cout << "FOOZLEBOOZEL 2" << std::endl;
        Local<Event::Event> event = Event::create(pc);
        Nan::Set(event, Nan::New("stream").ToLocalChecked(), media_stream_object);
...
NAN_METHOD(PeerConnection::GetLocalStreams)
{
  ...
  rtc::scoped_refptr<webrtc::StreamCollectionInterface> local_streams =
    self->_jinglePeerConnection->local_streams();
  for (size_t i = 0; i < local_streams->count(); i++) {
    Local<Object> media_stream_object = Nan::NewInstance(
      Nan::New(MediaStream::constructor), 0, nullptr).ToLocalChecked();
    MediaStream* media_stream = Nan::ObjectWrap::Unwrap<MediaStream>(media_stream_object);
    media_stream->setStream(local_streams->at(i));
    Nan::Set(array, i, media_stream_object);
  }
...
NAN_METHOD(PeerConnection::GetRemoteStreams)
{
  ...
  rtc::scoped_refptr<webrtc::StreamCollectionInterface> remote_streams =
    self->_jinglePeerConnection->remote_streams();
  for (size_t i = 0; i < remote_streams->count(); i++) {
    Local<Object> media_stream_object = Nan::NewInstance(
      Nan::New(MediaStream::constructor), 0, nullptr).ToLocalChecked();
    MediaStream* media_stream = Nan::ObjectWrap::Unwrap<MediaStream>(media_stream_object);
    media_stream->setStream(remote_streams->at(i));
    Nan::Set(array, i, media_stream_object);
  }
  info.GetReturnValue().Set(array);
  TRACE_END;
...
void PeerConnection::Init(rtc::scoped_refptr<webrtc::PeerConnectionFactoryInterface> peerConnectionFactory, Handle<Object> exports ) {
  ...
  tpl->PrototypeTemplate()->Set( Nan::New( "getLocalStreams" ).ToLocalChecked(),
    Nan::New<FunctionTemplate>( GetLocalStreams )->GetFunction() );

  tpl->PrototypeTemplate()->Set( Nan::New( "getRemoteStreams" ).ToLocalChecked(),
    Nan::New<FunctionTemplate>( GetRemoteStreams )->GetFunction() );
